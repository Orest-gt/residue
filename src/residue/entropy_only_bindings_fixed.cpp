...
PYBIND11_MODULE(residue, m) {
    ...
    // Simplified factory function - no extra parameters
    m.def("create_entropy_controller", &create_entropy_controller,
           "Create entropy controller instance",
           py::arg("num_bins") = 256, py::arg("entropy_threshold") = 0.1f);
    
    // Main convenience function - simplified
    m.def("compute_scaling", [](py::array_t<float> input, float threshold = 0.1f) {
        // Fast entropy calculation without creating controller instance
        ...
        auto controller = create_entropy_controller(256, threshold);
